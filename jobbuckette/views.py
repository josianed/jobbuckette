...
def edit_position_post(coid, posid):
    company = session.query(Company).get(coid)
    position = session.query(Position).get(posid)
    position.position_name = request.form.get("inputPositionName")
    position.date_due = request.form.get("inputDueDate")
    position.link_to_website = request.form.get("inputWebsite")
    session.commit()
    return redirect(url_for('positions', coid=coid))
