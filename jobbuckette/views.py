...
def edit_company_post(coid):
    company = session.query(Company).get(coid)
    company.name = request.form["inputCompanyName"]
    company.location = request.form["inputLocation"]
    company.industry = request.form["inputIndustry"]
    company.link_to_website = request.form["inputWebsite"]
    session.commit()
    return redirect(url_for('companies'))
